...
let test_try_parse_opt () =
  assert_equal ~label:"some" (Some (J.Encode.int 1)) @@ J.try_parse_opt {| 1 |};
  assert_equal ~label:"none" None @@ J.try_parse_opt {| [ |}

let test_try_parse_exn () =
  assert_equal ~label:"no exception" (J.Encode.int 1) @@ J.try_parse_exn {| 1 |};
  assert_raises
    ~label:"exception"
...
module TestEncode = struct
  ...
  let test_bool () =
    assert_equal ~label:"bool" ~equal (J.try_parse_exn {| true |}) @@ bool true
...
module TestDecode = struct
  ...
  let test_nullable () =
    assert_equal ~label:"int -> int" (Some 23) @@ nullable int (J.Encode.int 23);
  ...
  let test_list () =
    ...
    assert_raises
      ...
      (fun () ->
        ignore
        @@ (list (fun _ -> failwith "fail")) (J.Encode.list J.Encode.int [ 1 ]));
  ...
  let test_pair () =
    ...
    assert_raises
      ...
      (fun () ->
        ignore
        @@ (pair (fun _ -> failwith "fail") int) (J.try_parse_exn {| [4, 3] |}))
  ...
  let test_tuple4 () =
    ...
    @@ tuple4 string int float bool (J.try_parse_exn {| ["a", 3, 4.5, true] |});
    assert_raises
      ~label:"too small"
  ...
  let test_dict () =
    ...
    assert_raises
      ...
      (fun () ->
        ignore @@ (dict string) (J.try_parse_exn {| { "a": null, "b": null } |}));
    assert_raises
      ~label:"non-Decode_error exceptions in decoder should pass through"
      (Failure "fail")
      (fun () ->
        ignore
        @@ (dict (fun _ -> failwith "fail")) (J.try_parse_exn {| { "a": 0 } |}));
  ...
  let test_at () =
    ...
    @@ at
         [ "a"; "x"; "y" ]
         bool
         (J.try_parse_exn {| { "a": { "x" : { "y" : false } }, "b": false } |});
  ...
  let test_try_default () =
    assert_equal ~label:"boolean" 0 @@ (try_default 0 int) (J.Encode.bool true);
    ...
    assert_equal ~label:"object" 0 @@ (try_default 0 int) (J.Encode.object_ []);
